function [G,ASP] = gradient8(DEM,cellsize)
...
%     NEIGH(2,:) = true;
%     NEIGH(:,2) = true;
%     G = (DEM-imerode(DEM,NEIGH))/cellsize;
%     
%     % calculate along diagonal neighbors
%     NEIGH(:,:) = false;
%     NEIGH([1 5 9 3 7]) = true;
%     G = max(G,(DEM-imerode(DEM,NEIGH))/hypot(cellsize,cellsize));
%     
%     if flagnan
%         G(I) = nan;
%     end
...
% elseif nargout == 2;
    ...
    neighfun = cell(8,2);
    res2 = hypot(cellsize,cellsize);
    
    neighfun{1,1} = @(x) (x(2:end-1,2:end-1)-x(1:end-2,2:end-1))/cellsize;
    neighfun{2,1} = @(x) (x(2:end-1,2:end-1)-x(1:end-2,3:end))/res2;
    neighfun{3,1} = @(x) (x(2:end-1,2:end-1)-x(2:end-1,3:end))/cellsize;
    neighfun{4,1} = @(x) (x(2:end-1,2:end-1)-x(3:end,3:end))/res2;
    
    neighfun{5,1} = @(x) (x(2:end-1,2:end-1)-x(3:end,2:end-1))/cellsize;
    neighfun{6,1} = @(x) (x(2:end-1,2:end-1)-x(3:end,1:end-2))/res2;
    neighfun{7,1} = @(x) (x(2:end-1,2:end-1)-x(2:end-1,1:end-2))/cellsize;
